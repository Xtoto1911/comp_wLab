...
def normalization(image: numpy.ndarray, k: numpy.float32) -> numpy.ndarray:
    avarage = image.mean()
    ...
    return float_image.astype(numpy.uint8)
...
while True:
    key = cv2.waitKey(0)
    if key == ord('w') and k < 10:
        k = round(k+0.1,1) 
        print(f'k: {k}')
    elif key == ord('s') and k > 0.1:
        k = round(k-0.1,1) 
        print(f'k: {k}')
    elif key == 27:
        break
