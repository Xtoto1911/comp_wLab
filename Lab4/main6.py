...
insert = cv2.VideoCapture('chessboard.mp4')
width = insert.get(cv2.CAP_PROP_FRAME_WIDTH)
height = insert.get(cv2.CAP_PROP_FRAME_HEIGHT)
...
try:
    while True:
        ...
        if not success:  # кадр прочитать не удалось
            ...
            break
        success, insideframe = insert.read()
        if not success:  # кадр прочитать не удалось
            print('Video has ended.')
            break
        if warped is None:
        ...
        insert_corners = make_pattern(insideframe.shape)
        if success:  # нашли шаблон
            ...
            cv2.warpPerspective(  # проективное преобр. изображения
                insideframe,  # какое изображение преобразуем
